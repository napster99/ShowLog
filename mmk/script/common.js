...
function URLConfig(name, data) {
  ...
  switch(name) {
    ...
    case 'payOrder2':
      ...
      // 支付 last 
      case 'order_detai':
        return BASE_URL + 'cpond/order/info?' + getSignCode();

      case 'order_get_discount':
        return BASE_URL + 'cpond/order/discount?' + getSignCode();

      case 'order_submit':
        return BASE_URL + 'cpond/order/submit?' + getSignCode();

      case 'order_time':
        return BASE_URL + 'cpond/order/check?' + getSignCode();

      case 'order_pay':
        return BASE_URL + 'cpond/order/checkout1?' + getSignCode();

    case 'jpushUrl':
        return 'http://dwz.cn/query.php?' + getSignCode();
   }
...
//分享弹窗
window.sharePop = function(title, datas) {
 ...
 */
function showImgView(picsAttr,thumbImg,index) {
  api.openWin({
    'name' : 'img-view',
